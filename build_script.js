...
export default function CycloneTracker({ lat, lon, locationName = 'your area' }) {
  ...
  const fetchAlerts = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(\\\/api/extreme-alerts?lat=\\\&lon=\\\\\\);
        ...
        <div className="flex gap-2 flex-wrap">
          {alerts.map((a, i) => (
            <button key={i} onClick={() => setSelected(i)}
              className={\\\lex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border transition-all
                \\\\\\}>
              <span>{a.icon}</span>
      ...
      <div className="bg-[#2A2346]/95 backdrop-blur-xl border border-white/5 rounded-3xl p-6 relative overflow-hidden flex flex-col xl:flex-row gap-8 shadow-[0_15px_40px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex-1 text-white">
          ...
          <div className="grid grid-cols-2 gap-y-8 gap-x-4 mb-8">
            ...
            <div>
              ...
              <div className="text-2xl font-bold tracking-wide">{active.riskWindow || 'Live Now'}</div>
            </div>
            <div>
              <div className="text-[11px] uppercase font-bold text-white/40 tracking-widest mb-1.5 flex items-center gap-1.5">
              ...
              <div className={\\\	ext-2xl font-bold capitalize \\\\\\}>{active.severity}</div>
            </div>
          </div>
        ...
        <div className="w-full xl:w-[45%] h-[320px] xl:h-auto min-h-[350px] rounded-2xl overflow-hidden border border-white/10 relative">
          <iframe
            ...
            src={\\\https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=mm&metricTemp=%C2%B0C&metricWind=km/h&zoom=9&overlay=\\\&product=ecmwf&level=surface&lat=\\\&lon=\\\&detailLat=\\\&detailLon=\\\&marker=true\\\}
            frameBorder="0"
            style={{ border: 0 }}
...
fs.writeFileSync('src/components/CycloneTracker.jsx', content);