...
export default function CycloneTracker({ lat, lon, locationName = "your area", refreshTrigger }) {
  ...
  return (
    <div className="flex flex-col gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          {alerts.map((a, i) => (
            <button key={i} onClick={() => setSelected(i)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${i === selected ? "bg-black/40 border-white/20 text-indigo-400" : "bg-transparent border-white/10 opacity-50 hover:opacity-100"}`}>
              <span>{a.icon}</span>
      ...
      {/* Main alert card - THEME AWARE DESIGN */}
      <div className="glass-panel border-white/10 rounded-3xl p-6 relative overflow-hidden flex flex-col xl:flex-row gap-8 shadow-2xl">
        
        {/* Left: details */}
        <div className="flex-1">
          <div className="flex items-start gap-4 mb-6">
            <div className="w-14 h-14 rounded-full flex-shrink-0 flex items-center justify-center border border-red-500/30 bg-red-500/10 text-red-500 text-2xl shadow-inner">
              {active.icon}
            </div>
            <div className="flex-1 mt-1">
              <h3 className="text-[22px] font-bold leading-tight mb-2 tracking-wide">{active.title} in {locationName}</h3>
              <p className="text-sm opacity-80 leading-relaxed pr-4">{active.description}</p>
            </div>
            <div className="flex-shrink-0">
              <span className="text-[10px] font-bold tracking-widest uppercase border px-3 py-1.5 rounded bg-red-500/10 text-red-500 border-red-500/30 shadow-[0_0_10px_rgba(239,68,68,0.2)]">
                {active.severity}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-y-8 gap-x-4 mb-8">
            <div>
              <div className="text-[11px] uppercase font-bold opacity-50 tracking-widest mb-1.5 flex items-center gap-1.5">
          ...
          <div className="border border-red-500/30 bg-red-500/5 rounded-xl p-5 mb-5 shadow-inner">
            <div className="flex items-center gap-2 text-[11px] font-bold text-red-500 tracking-widest uppercase mb-3">
